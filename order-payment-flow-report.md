...
## 6. المشاكل والملاحظات التي يجب معالجتها

### 6.1 مشكلة صيغة التنوعات (Variations) — تم حلها من التطبيق

| البند | التفاصيل |
|-------|----------|
| **المشكلة** | الباك إند كان يرجع `variation.values: []` فارغاً، فلا يظهر الخيار المختار (مثل "رز شعبي") في تفاصيل الطلب. |
| **السبب (من التطبيق)** | التطبيق كان يرسل `values` ككائن واحد بصيغة `{ "label": ["رز شعبي"] }` بينما الباك إند يتوقع **مصفوفة كائنات** بصيغة `[{ "label": "رز شعبي", "optionPrice": "23.5", "optionCalories": "1563" }]`. |
| **ما تم في التطبيق** | تم إرسال `values` كمصفوفة كائنات (كل عنصر: `label`, `optionPrice`, `optionCalories`) عبر `OrderVariationValueItem` و `valueItems` في `place_order_body.dart` و `confirm_button_view.dart`. |
| **النتيجة** | الباك إند أصبح يحفظ ويعيد التنوعات بشكل صحيح. |

---

### 6.2 مشكلة سعر الصنف في الاستجابة (price ≠ المُرسل) — تعويض من التطبيق، والإصلاح الكامل من الباك إند

| البند | التفاصيل |
|-------|----------|
| **المشكلة** | التطبيق يرسل في الـ cart `price: "23.5"` لكن استجابة **Track** و **Order Details** ترجع في كل صنف طلب `price: 28.5` (أو قيم أخرى مختلفة). |
| **السبب المحتمل** | الباك إند لا يستخدم حقل `price` المرسل من التطبيق عند حفظ تفاصيل الطلب، ويحسب سعر الوحدة بنفسه (مثلاً بإضافة ضريبة أو من جداول أخرى)، فيُخزّن ويُرجع قيمة خاطئة. |
| **ما تم في التطبيق (تعويض)** | تم استخدام **سعر التنوع** عند العرض والحساب عندما يكون متوفراً: إضافة `effectiveUnitPrice` في `OrderDetailsModel` (يأخذ `optionPrice` من `variation.values[0]` إن وُجد، وإلا `price`). استخدام `effectiveUnitPrice` في: حساب `itemsPrice` في شاشة تفاصيل الطلب، عرض سعر الوحدة في `details_view.dart`، وإعادة الطلب في `order_item.dart`. |
| **ما يجب معالجته (الباك إند)** | يجب أن يحفظ الباك إند في جدول تفاصيل الطلب **نفس** قيمة `price` المرسلة في كل عنصر من `cart[]` ويُرجعها في الاستجابة، دون إعادة حساب أو استبدال بسعر من جداول المنتجات/الضريبة. |

---

### 6.3 ملاحظات خاطئة أو محتملة يجب تجنبها

| الملاحظة | التصحيح |
|----------|----------|
| **إرسال `price` كنص** | التطبيق يرسل `cart[].price` كـ string (مثل `"23.5"`). إذا كان الباك إند يتوقع number، قد يحدث تحويل خاطئ أو تجاهل. يُفضّل التأكد من وثائق الـ API؛ إن لزم، إرسال `price` كـ number في الـ body. |
| **إرسال `tax_amount: 0` لكل صنف** | التطبيق يرسل دائماً `tax_amount: 0` للعناصر. إذا كان الباك إند يفسّر ذلك أو يعتمد عليه لحساب السعر النهائي، قد يؤثر على النتيجة. التأكد مع الباك إند أن إرسال 0 صحيح في هذه الحالة. |
| **اعتماد العرض على `order_details.price` فقط** | بعد تطبيق التعويض، التطبيق يعتمد على `effectiveUnitPrice` (من التنوع إن وُجد) للعرض والحساب. إن لم يكن للصنف تنوعات، يُستخدم `price` من الاستجابة. |

---

### 6.4 ملخص الحالة الحالية

| البند | الحالة |
|-------|--------|
| صيغة التنوعات (إرسال واستقبال) | ✅ تم إصلاحها من التطبيق؛ الباك إند يرجع التنوعات بشكل صحيح. |
| عرض سعر الوحدة والمجموع في التطبيق | ✅ تعويض عبر `effectiveUnitPrice` عند وجود تنوعات. |
| تخزين الباك إند لـ `price` المرسل | ⚠️ ما زال يُرجع قيمة مختلفة (مثل 28.5 بدل 23.5)؛ الإصلاح الكامل يتطلب تعديل الباك إند لاستخدام `cart[].price` كما يُرسل. |
